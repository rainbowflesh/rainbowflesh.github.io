...
<template>
  ...
  <div class="container">
    <div class="content">
      ...
      <ol>
        <li>
          <a href="#%E8%AE%A1%E7%AE%97%E6%A1%86%E6%9E%B6-spark"
            >计算框架 Spark</a
          >
          <ol>
            <li>
              ...
              <ol>
                <li>
                  <a
                    href="#spark-%E4%B8%AD-job-stage-task-%E5%88%86%E5%88%AB%E6%98%AF%E4%BB%80%E4%B9%88"
                ...
                <li>
                  ...
                  <ol>
                    <li>
                      <a
                        href="#reducebykey-%E5%92%8C-groupbykey-%E7%9A%84%E5%8C%BA%E5%88%AB"
                        >reduceByKey 和 groupByKey 的区别</a
                      >
                    </li>
                    <li>
                      <a
                        href="#rdd-%E7%9A%84%E5%88%9B%E5%BB%BA%E6%96%B9%E5%BC%8F"
                        >RDD 的创建方式</a
                      >
                    </li>
                    <li>
                      <a
                        href="#rdd-%E7%9A%84%E4%BE%9D%E8%B5%96%E5%85%B3%E7%B3%BB"
                        >RDD 的依赖关系</a
                      >
                    </li>
                    <li>
                    ...
                    <li>
                      <a
                        href="#rdd-%E7%9A%84%E7%BC%93%E5%AD%98%E6%8C%81%E4%B9%85%E5%8C%96%E6%9C%BA%E5%88%B6"
                        >RDD 的缓存持久化机制</a
                      >
                    </li>
                  </ol>
                ...
                <li>
                  ...
                  <ol>
                    ...
                    <li>
                      <a href="#%E6%8F%90%E9%AB%98%E5%B9%B6%E8%A1%8C%E5%BA%A6"
                        >提高并行度</a
                      >
                    </li>
                    <li>
                      <a
                        href="#rdd-%E7%9A%84%E9%87%8D%E7%94%A8%E5%92%8C%E6%8C%81%E4%B9%85%E5%8C%96"
                        >RDD 的重用和持久化</a
                      >
                    </li>
                    <li>
                    ...
                    <li>
                      <a
                        href="#%E4%BD%BF%E7%94%A8-map-side-%E9%A2%84%E5%85%88%E8%81%9A%E5%90%88-shuffle"
                        >使用 map-side 预先聚合 Shuffle</a
                      >
                    </li>
                    <li>
                      <a
                        href="#%E4%BD%BF%E7%94%A8%E9%AB%98%E6%80%A7%E8%83%BD%E7%AE%97%E5%AD%90"
                        >使用高性能算子</a
                      >
                    </li>
                  </ol>
                ...
                <li>
                  <a href="#spark-%E7%9A%84%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6"
                    >Spark 的工作机制</a
                  >
                </li>
                <li>
                  <a
                    href="#spark-%E6%A8%A1%E5%9D%97%E9%83%BD%E6%9C%89%E5%93%AA%E4%BA%9B"
                    >Spark 模块都有哪些</a
                  >
                </li>
                <li>
                  <a
                    href="#dstream-%E5%92%8C-dstreamgraph-%E7%9A%84%E5%8C%BA%E5%88%AB"
      ...
      <ul>
        ...
        <li>
          Spark 通过 RDD 算子来运算, 具有转换与动作两种操作,
          可以把运算结果缓存在内存, 再计算出来.
        </li>
        <li>Spark 还拥有容错机制 Linage 算子, 可以把失败的任务重新执行.</li>
      </ul>
      <h3
        id="spark-%E4%B8%AD-job-stage-task-%E5%88%86%E5%88%AB%E6%98%AF%E4%BB%80%E4%B9%88"
      >
        Spark 中 job, stage, Task 分别是什么
      </h3>
      <ul>
        <li>
          job 是提交给 Spark 的任务, 会被拆分为多组 Task, 每一次数据的 Shuffle
          都会产生一个 stage; 每次 action 都会产生一个 job.
        </li>
        <li>
          stage 是 job 执行需要的阶段. 划分 stage 使每一个 stage 只有窄依赖,
          可以实现流计算, 同时每一个 Task 对应一个分区, 增加了 Task
          的并行运行量.
        </li>
        <li>
          Task 是 stage 的任务执行单元, 通常和 RDD 的 Partition 数量相同,
          只是处理一个 Partition 上的数据; 每个 Task 都是一个 JVM 实例, JVM
          的开启与销毁会降低系统运行效率; Task 是任务的最小单位, 最终运行在
          Executor 中.
        </li>
      </ul>
      <h3 id="%E4%BB%80%E4%B9%88%E6%98%AF-rdd">什么是 RDD</h3>
      <p>
        RDD 就是弹性分布式数据集 Resilient Distributed Datasets, Spark
        的数据抽象概念, 是一种不可变分布式对象集合; 每个 RDD 都被分为多个分区,
        运行在集群不同节点上, 拥有多种不同的 RDD 算子.
      </p>
      <ul>
        <li>
          <p>
            transformation 中有 map().filter(), flatMap(), groupByKey(),
            reduceByKey(), sortByKey(), join(), cogroup(), distinct(), sample(),
            union(), intersection(), cartesian(), pipe() 等.
          </p>
        </li>
        <li>
          <p>
            action 中有 mapPartitions(), foreach(), collect(), count(), take(),
            top(), takeOrdered(), saveAsTextFile(), saveAsObjectFile(),
            persist(), unpersist(), checkpoint(), checkpointFile(),
            checkpointRDD(), getCheckpointFile() 等.
          </p>
        </li>
      </ul>
      <h4 id="reducebykey-%E5%92%8C-groupbykey-%E7%9A%84%E5%8C%BA%E5%88%AB">
        reduceByKey 和 groupByKey 的区别
      </h4>
      <ul>
        <li>
          reduceByKey 会在结果处发送至 Reducer 前, 对每个 Mapper 在本地进行
          Merge; 类似于 MapReduce 中的 Combiner, 这样做使 Map 端进行一次 Reduce
          后数据量会大幅度减小, 从而降低传输量, 保证 Reduce
          能够更快的进行结果计算.
        </li>
        <li>
          groupByKey 会对每一个 RDD 中的 value 值进行聚合形成序列 Iterator,
          此操作发生在 Reduce 上, 所以会传输所有的数据, 因而造成资源浪费,
          同时数据流很大的时候还会造成内存溢出 OutOfMemoryError.
        </li>
      </ul>
      <h4 id="rdd-%E7%9A%84%E5%88%9B%E5%BB%BA%E6%96%B9%E5%BC%8F">
        RDD 的创建方式
      </h4>
      <p>使用 <code>makeRDD</code> 通过集合创建, 由本地核数决定分区数量.</p>
      <p>使用外部数据源 HDFS 的时候, 由 Block 数量决定分区数, 最低是两个.</p>
      <p>
        由另一个 RDD 得出的结果创建, 即转换时创建, 会根据父 RDD 的 reduceTask
        数量决定分区.
      </p>
      <h4 id="rdd-%E7%9A%84%E4%BE%9D%E8%B5%96%E5%85%B3%E7%B3%BB">
        RDD 的依赖关系
      </h4>
      <ol>
      ...
      <h4
        id="%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A6%81%E5%88%92%E5%88%86%E4%BE%9D%E8%B5%96%E5%85%B3%E7%B3%BB"
      >
        为什么要划分依赖关系
      </h4>
      ...
      <h4
        id="rdd-%E7%9A%84%E7%BC%93%E5%AD%98%E6%8C%81%E4%B9%85%E5%8C%96%E6%9C%BA%E5%88%B6"
      >
        RDD 的缓存持久化机制
      </h4>
      ...
      <ol>
        <li>
          ...
          <ul>
            ...
            <li>
              persist 定义了多种数据储存策略, 如磁盘内存多副本.
              数据存储在内存中会产生内存溢出, 断点失效等问题,
              不能保证数据准确和安全.
            </li>
        ...
        <li>
          ...
          <ul>
            ...
            <li>
              会改变 RDD 依赖关系, 数据丢失后无法通过依赖关系恢复数据;
              程序执行结束后不会回收.
            </li>
          </ul>
        </li>
      ...
      <pre
        class="hljs"
      ><code><div><span class="hljs-meta">#</span><span class="bash"> 配置 Executor 数量</span>
 --num-Executors 3
...
<span class="hljs-meta">#</span><span class="bash"> 配置 Executor cpu 核心使用个数, 单位 个</span>
 --Executor-cores n
      ...
      <ul>
        <li>
          设置 Task 数量: <code>spark.default.parallelism</code>, 设置后会在
          Shuffle 过程中应用; 可以通过构造 SparkConf 对象时设置
          <code
            >new
            SparkConf().set(&quot;spark.default.parallelism&quot;,&quot;500&quot;)</code
          >
        </li>
        <li>
          设置 RDD 的 Partition 数量, 使用
          <code>rdd.repartition</code> 重新分区, 会生成新的 RDD 并使分区变大,
          同时 Task 也会变多.
        </li>
        <li>
          设置参数 <code>sql.Shuffle.partitions = n</code>, 适当增大 n 的值.
        </li>
      </ul>
      <h4
        id="rdd-%E7%9A%84%E9%87%8D%E7%94%A8%E5%92%8C%E6%8C%81%E4%B9%85%E5%8C%96"
      >
        RDD 的重用和持久化
      </h4>
      <ul>
        <li>
          适当的重用和持久化 RDD 有利于减少代码重复和重读, 通常是 ca`che 和
          persist 方式持久化.
        </li>
        <li>
          持久化过程中适当的使用序列化可以减少数据的大小, 从而降低内存和 cpu
          占用率, 减少网络 IO 消耗和磁盘占用; 序列化后读取数据需要反序列化,
          多了一次处理.
        </li>
      ...
      <blockquote>
        <p>
          一个副本时服务宕机了就得重新计算, 持久化每个数据单元,
          并多做冗余互备来容错. 适合大内存场景.
          <code>StorageLevel.MEMORY_ONLY_2</code>.
        </p>
      </blockquote>
      <h4
        id="%E9%80%82%E5%BD%93%E5%B9%BF%E6%92%AD%E5%A4%9A%E6%AC%A1%E4%BD%BF%E7%94%A8%E7%9A%84%E5%8F%98%E9%87%8F"
      >
        适当广播多次使用的变量
      </h4>
      <ul>
        <li>
          ...
          <ul>
            ...
            <li>
              使用广播变量后, 50 个 Executor 创建 50 个数据副本,
              并且从就近节点的 BlockManager 上 PULL 到一个节点上,
              广播变量还可以减少网络 IO.
            </li>
          </ul>
        </li>
      ...
      <blockquote>
        <p>
          Executor 的 BlockManager 就可以拉取该广播变量的副本获取具体的数据,
          获取广播变量中的值可以通过调用其 value 方法:
        </p>
      </blockquote>
      ...
      <blockquote>
        ...
        <ol>
          ...
          <li>
            Spark 中因为算子的真正逻辑是发送给 Executor 运行的, Executor
            需要引用外部变量时, 可以使用广播变量.
          </li>
          <li>广播变量只能在 Driver 上定义和修改变量值.</li>
          <li>
            若 Executor 调用了 Driver 的变量, 不使用广播时有几个 Task
            就要创建几个 Driver 的变量副本. 用了广播后只创建一个副本.
          </li>
        </ol>
      </blockquote>
      <h4
      ...
      <pre
        class="hljs"
      ><code><div><span class="hljs-comment">// join 会产生 Shuffle</span>
...
<span class="hljs-comment">// 再根据业务将 rdd1 与 rdd2 可连接的数据拼接成 String 或 Tuple</span>
      ...
      <h4
        id="%E4%BD%BF%E7%94%A8-map-side-%E9%A2%84%E5%85%88%E8%81%9A%E5%90%88-shuffle"
      >
        使用 map-side 预先聚合 Shuffle
      </h4>
      <ul>
        <li>
          一定要使用 Shuffle 的场景下, 无法使用 map 类算子替代, 则尽量使用
          map-side 预聚合算子, 即在各节点对相同的 key 聚合, 类似 MapReduce 的
          Combiner. 聚合后 PULL 数据就能节省磁盘和网络 IO 开销.
        </li>
        <li>
          若条件允许, 使用 <code>reduceByKey()</code> 或者
          <code>aggregateByKey()</code> 来替代 <code>groupByKey()</code>,
          前者可以使用自定义函数聚合 key, 后者的会使全量的数据在节点间分发,
          性能相对较差.
        </li>
      </ul>
      <h4 id="%E4%BD%BF%E7%94%A8%E9%AB%98%E6%80%A7%E8%83%BD%E7%AE%97%E5%AD%90">
        使用高性能算子
      </h4>
      <ol>
        <li>
          ...
          <ul>
            <li>
              <code>mapPartitions()</code> 类的算子每次调用会处理 Partition
              中所有的数据, 相比单条处理效率高些, 但会占用大量内存.
            </li>
          </ul>
        </li>
        <li>
        ...
        <li>
          使用 <code>repartitionAndSortWithinPartitions()</code> 替代
          <code>repartition()</code> 与 <code>sort()</code> 类操作
          <ul>
            <li>
              Spark 官方推荐在
              <code>repartition()</code> 后的排序场景下使用这个名字很长的算子,
              该算子可以并行执行重新分区的 shuffle 和排序.
            </li>
          </ul>
        </li>
        <li>
          ...
          <ul>
            <li>
              fastutil 是 java 标准集合框架的扩展库, 提供了特殊的 map, set,
              list, queue; fastutil 能减少内存开销.
              除了一些使用外部变量的场景外, 对付某种较大集合时也可以用 fastutil
              改写外部变量.
            </li>
          </ul>
        </li>
      ...
      <ul>
        <li>
          利用 checkPoint 机制, 每次 SparkSteaming 消费 Kafka 后将 Kafka Offsets
          更新到 checkPoint, 重启后读取 checkPoint 就能继续.
        </li>
        <li>
          在 SparkStreaming 中启用预写日志, 同步保存所有收到的 Kafka 数据到 HDFS
          中, 故障后方便恢复到上次未知, 代价是储存空间占用.
        </li>
      </ul>
      <h3 id="spark-%E7%B4%AF%E5%8A%A0%E5%99%A8">Spark 累加器</h3>
      <p>
        累加器相当于统筹大变量, 常用于计数统计工作. 累加器通常被视为 RDD 的
        map().filter() 操作的副产物.
      </p>
      <h3 id="spark-%E7%9A%84%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6">
        Spark 的工作机制
      </h3>
      <ol>
        ...
        <li>
          DAGScheduler 按照 RDD 的依赖关系划分 stage, 输入 Task, Scheduler
          去划分 Task, .
        </li>
        <li>
          set 分发到各个节点的 Executor, 并以多线程的方式执行 Task, 一个线程一个
          Task, 任务结束后根据类型返回结果.
        </li>
      </ol>
      <h3 id="spark-%E6%A8%A1%E5%9D%97%E9%83%BD%E6%9C%89%E5%93%AA%E4%BA%9B">
        Spark 模块都有哪些
      </h3>
      <h3 id="dstream-%E5%92%8C-dstreamgraph-%E7%9A%84%E5%8C%BA%E5%88%AB">
