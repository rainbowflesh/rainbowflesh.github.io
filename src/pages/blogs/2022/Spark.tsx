        ...
        <div className="container">
          <div className="content">
                    ...
                    <ol>
                      ...
                      <li>
                        ...
                        <ol>
                          ...
                          <li>
                            <a href="#%E4%B8%BA%E4%BB%80%E4%B9%88%E8%A6%81%E5%88%92%E5%88%86%E4%BE%9D%E8%B5%96%E5%85%B3%E7%B3%BB">
                              为什么要划分依赖关系
                            </a>
                          </li>
                          <li>
                      ...
                      <li>
                        ...
                        <ol>
                          <li>
                            <a href="#%E4%BF%AE%E6%94%B9%E5%BA%8F%E5%88%97%E5%8C%96%E6%9C%BA%E5%88%B6%E5%8E%8B%E7%BC%A9%E6%95%B0%E6%8D%AE%E9%87%8F">
                              修改序列化机制压缩数据量
                            </a>
                          </li>
                          <li>
                            <a href="#%E6%A0%B9%E6%8D%AE%E4%BD%9C%E4%B8%9A%E7%8E%AF%E5%A2%83-%E6%8F%90%E4%BA%A4%E4%BB%BB%E5%8A%A1%E6%97%B6%E8%B0%83%E6%95%B4-spark-submitsh-%E5%8F%82%E6%95%B0">
                          ...
                          <li>
                            <a href="#%E9%80%82%E5%BD%93%E5%B9%BF%E6%92%AD%E5%A4%9A%E6%AC%A1%E4%BD%BF%E7%94%A8%E7%9A%84%E5%8F%98%E9%87%8F">
                              适当广播多次使用的变量
                            </a>
                          </li>
                          <li>
                            <a href="#%E9%81%BF%E5%85%8D%E4%BA%A7%E7%94%9F-shuffle-%E5%8F%AF%E4%BB%A5%E5%87%8F%E5%B0%91%E7%BD%91%E7%BB%9C-io-%E5%92%8C%E5%88%86%E5%8C%BA%E9%97%B4%E4%BC%A0%E8%BE%93%E6%B6%88%E8%80%97">
                          ...
                          <li>
                            <a href="#%E4%BD%BF%E7%94%A8%E9%AB%98%E6%80%A7%E8%83%BD%E7%AE%97%E5%AD%90">
                              使用高性能算子
                            </a>
                          </li>
                        </ol>
                      ...
                      <li>
                        <a href="#sparkstreaming-%E5%9C%A8%E6%B6%88%E8%B4%B9-kafka-%E6%97%B6%E4%B8%8B%E7%BA%BF-%E5%A6%82%E4%BD%95%E4%BF%9D%E8%AF%81%E9%87%8D%E5%90%AF%E5%90%8E%E7%BB%A7%E7%BB%AD%E6%B6%88%E8%B4%B9">
            ...
            <ul>
              ...
              <li>
                Task 是 stage 的任务执行单元, 通常和 RDD 的 Partition 数量相同, 只是处理一个 Partition 上的数据; 每个
                Task 都是一个 JVM 实例, JVM 的开启与销毁会降低系统运行效率; Task 是任务的最小单位, 最终运行在 Executor
                中.
              </li>
            </ul>
            <h3 id="%E4%BB%80%E4%B9%88%E6%98%AF-rdd">什么是 RDD</h3>
            <p>
              RDD 就是弹性分布式数据集 Resilient Distributed Datasets, Spark 的数据抽象概念, 是一种不可变分布式对象集合;
              每个 RDD 都被分为多个分区, 运行在集群不同节点上, 拥有多种不同的 RDD 算子.
            </p>
            <ul>
              <li>
                <p>
                  transformation 中有 map().filter(), flatMap(), groupByKey(), reduceByKey(), sortByKey(), join(),
                  cogroup(), distinct(), sample(), union(), intersection(), cartesian(), pipe() 等.
                </p>
              </li>
              <li>
                <p>
                  action 中有 mapPartitions(), foreach(), collect(), count(), take(), top(), takeOrdered(),
                  saveAsTextFile(), saveAsObjectFile(), persist(), unpersist(), checkpoint(), checkpointFile(),
            ...
            <p>
              使用 <code>makeRDD</code> 通过集合创建, 由本地核数决定分区数量.
            </p>
            ...
            <h4 id="rdd-%E7%9A%84%E7%BC%93%E5%AD%98%E6%8C%81%E4%B9%85%E5%8C%96%E6%9C%BA%E5%88%B6">
              RDD 的缓存持久化机制
            </h4>
            ...
            <h4 id="%E4%BF%AE%E6%94%B9%E5%BA%8F%E5%88%97%E5%8C%96%E6%9C%BA%E5%88%B6%E5%8E%8B%E7%BC%A9%E6%95%B0%E6%8D%AE%E9%87%8F">
              修改序列化机制压缩数据量
            </h4>
            <p>通过 Kryo 调整序列化性能.</p>
            <ul>
            ...
            <h4 id="%E6%A0%B9%E6%8D%AE%E4%BD%9C%E4%B8%9A%E7%8E%AF%E5%A2%83-%E6%8F%90%E4%BA%A4%E4%BB%BB%E5%8A%A1%E6%97%B6%E8%B0%83%E6%95%B4-spark-submitsh-%E5%8F%82%E6%95%B0">
              根据作业环境, 提交任务时调整 spark-submit.sh 参数
            </h4>
            <pre className="hljs">
              <code>
                <div>
                  ...
                  <span className="bash"> 配置 Executor 数量</span>
                  {"\n"} --num-Executors 3{"\n"}
                  <span className="hljs-meta">#</span>
                  <span className="bash"> 配置 driver 内存, 单位 gb</span>
                  {"\n"} --driver-memory ng{"\n"}
                  <span className="hljs-meta">#</span>
                  <span className="bash"> 配置 Executor 的内存大小, 单位 gb</span>
                  {"\n"} --Executor-memory n{"\n"}
                  <span className="hljs-meta">#</span>
                  <span className="bash"> 配置 Executor cpu 核心使用个数, 单位 个</span>
                  {"\n"} --Executor-cores n{"\n"}
                </div>
              </code>
            </pre>
            ...
            <ul>
              <li>
                设置 Task 数量: <code>spark.default.parallelism</code>, 设置后会在 Shuffle 过程中应用; 可以通过构造
                SparkConf 对象时设置
            ...
            <ul>
              <li>
                ...
                <ul>
                  ...
                  <li>
                    使用广播变量后, 50 个 Executor 创建 50 个数据副本, 并且从就近节点的 BlockManager 上 PULL
                    到一个节点上, 广播变量还可以减少网络 IO.
                  </li>
                </ul>
              </li>
            </ul>
            ...
            <pre className="hljs">
              <code>
                <div>
                  val broadcastArray:Broadcast[Array[Int]]=sc.broadcast(Array(<span className="hljs-number">1</span>,
                  <span className="hljs-number">2</span>,<span className="hljs-number">3</span>,
                  <span className="hljs-number">4</span>,<span className="hljs-number">5</span>,
                  <span className="hljs-number">6</span>)){"\n"}
                  {"        "}sc.broadcast(Array(<span className="hljs-number">1</span>,
                  <span className="hljs-number">2</span>,<span className="hljs-number">3</span>,
                  <span className="hljs-number">4</span>,<span className="hljs-number">5</span>,
                  <span className="hljs-number">6</span>)){"\n"}
                </div>
            ...
            <blockquote>
              <p>
                Executor 的 BlockManager 就可以拉取该广播变量的副本获取具体的数据, 获取广播变量中的值可以通过调用其
                value 方法:
              </p>
            </blockquote>
            <pre className="hljs">
              <code>
                <div>val array:Array[Int]=broadcastArray.value{"\n"}</div>
              </code>
            </pre>
            <blockquote>
              <p>注意:</p>
              <ol>
                ...
                <li>
                  Spark 中因为算子的真正逻辑是发送给 Executor 运行的, Executor 需要引用外部变量时, 可以使用广播变量.
                </li>
            ...
            <h4 id="%E9%81%BF%E5%85%8D%E4%BA%A7%E7%94%9F-shuffle-%E5%8F%AF%E4%BB%A5%E5%87%8F%E5%B0%91%E7%BD%91%E7%BB%9C-io-%E5%92%8C%E5%88%86%E5%8C%BA%E9%97%B4%E4%BC%A0%E8%BE%93%E6%B6%88%E8%80%97">
              避免产生 Shuffle 可以减少网络 IO 和分区间传输消耗
            </h4>
            <pre className="hljs">
              <code>
                <div>
                  <span className="hljs-comment">// join 会产生 Shuffle</span>
                  {"\n"}
                  <span className="hljs-comment">
                    // 两个 RDD 相同的 key 需要通过网络 PULL 到一个节点上, 由一个 Task 执行 join
                  </span>
                  {"\n"}
                  <span className="hljs-comment">// val rdd3 = rdd1.join(rdd2)</span>
                  {"\n"}
                  <span className="hljs-comment">// 广播变量和 Map 搭配的 join 操作不会产生 Shuffle</span>
                  {"\n"}val rdd2Data=rdd2.collect(){"\n"}
                  {"        "}val rdd2DataBroadcast=sc.broadcast(rdd2Data){"\n"}
                  <span className="hljs-comment">// 每个 Executor 的内存都会缓存一份 rdd2 的全量数据</span>
                  {"\n"}
                  <span className="hljs-comment">// 需要结合实际硬件性能考虑广播</span>
                  {"\n"}
                  <span className="hljs-comment">
                  ...
                  {"\n"}
                  <span className="hljs-comment">// 再根据业务将 rdd1 与 rdd2 可连接的数据拼接成 String 或 Tuple</span>
                  {"\n"}
            ...
            <ul>
              <li>
                一定要使用 Shuffle 的场景下, 无法使用 map 类算子替代, 则尽量使用 map-side 预聚合算子, 即在各节点对相同的
                key 聚合, 类似 MapReduce 的 Combiner. 聚合后 PULL 数据就能节省磁盘和网络 IO 开销.
              </li>
              <li>
                若条件允许, 使用 <code>reduceByKey()</code> 或者 <code>aggregateByKey()</code> 来替代
                <code>groupByKey()</code>, 前者可以使用自定义函数聚合 key, 后者的会使全量的数据在节点间分发,
                性能相对较差.
              </li>
            </ul>
            ...
            <ol>
              ...
              <li>
                ...
                <ul>
                  <li>
                    Spark 官方推荐在
                    <code>repartition()</code> 后的排序场景下使用这个名字很长的算子, 该算子可以并行执行重新分区的
                    shuffle 和排序.
                  </li>
                </ul>
              </li>
            ...
            <h3 id="sparkstreaming-%E5%9C%A8%E6%B6%88%E8%B4%B9-kafka-%E6%97%B6%E4%B8%8B%E7%BA%BF-%E5%A6%82%E4%BD%95%E4%BF%9D%E8%AF%81%E9%87%8D%E5%90%AF%E5%90%8E%E7%BB%A7%E7%BB%AD%E6%B6%88%E8%B4%B9">
              SparkStreaming 在消费 Kafka 时下线, 如何保证重启后继续消费
            </h3>
            <ul>
              <li>
