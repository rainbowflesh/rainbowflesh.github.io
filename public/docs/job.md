...
- Linux 子系统巴拉巴拉
  ...
  - 系统调用接口: 用户程序通过软件中断后, 调用系统内核提供的功能, 这个在用户空间和内核提供的服务之间的接口称为系统调用.
  - 进程管理子系统, 在用户空间，进程是由进程标示符（PID）表示; 在 linux 内核空间，每个进程都有一个独立的数据结构，用来保存该进程的 ID、优先级、地址的空间等信息，这个结构也被称做进程控制块（Process Control Block）。所谓的进程管理就是对进程控制块的管理。
  - 内存管理, 虚拟内存, 虚拟地址映射, 段页机制, 缺页中断, 内存的分配管理, 伙伴系统
  - 文件系统, 虚拟文件系统, zfsxfsextfatbrtfsrtfsapplefs
  - 网络接口 NIC
  - 进程通信

    1. Pipeline

       - 匿名管道：父子进程之间才能通信, 所有的进程都是有另一个进程创建的, 父进程在创建子进程的时候, 子进程会完全拷贝父进程, 其中就会拷贝父进程管道的位置
       - 命名管道：无亲缘关系的进程也能通信.
    ...
    2. 信号, 由内核发送
       ...
       信号的本质：修改程序的 pcb（进程控制块（数据结构）保存着进程的元数据：pid、有效用户、打开的文件等）

    3. 信号量, 一种锁机制, 可以规定同一时刻有几个程序可以访问这个共享内存
    4. 共享内存, 最快的进程通信方式
