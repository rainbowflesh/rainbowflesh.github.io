import{B as s}from"./BackToTopButton.a5a2f696.js";import{d as a,o as n,c as e,b as t,F as l,e as p}from"./index.8f841a57.js";const c="/images/blogs/IMG20220929185104.jpg",r=p('<title>Blog Tello Hack</title><div class="container"><div class="content"><h1 id="a-way-to-hack-dji-tello">A way to hack Dji Tello</h1><p> I got an R&amp;D job few weeks ago that required me to hack multiple drones, also known as <em>Counter-Unmanned Aircraft Swarm System</em>. I&#39;m in charge of confrontation and simulation work, but this is my first foray into the field, so first and foremost, I need to understand how to hack a single simple drone. </p><h2 id="system-design">System design</h2><p> Using a <strong>DJI Tello</strong> drone play victim role, a <strong>Rock Pi</strong> as attacker program carrier, and a <strong>GameSir T1d</strong> as attacker controller. </p><p><img src="'+c+`"></p><p>Two Wi-Fi cards are also required, one of which must be in monitor mode.</p><blockquote><p> I chose the RK3399 chip as the card, but the RockPi pre-built Ubuntu Focal <strong>didn&#39;t enable</strong> this driver by default; choose a right device make your life easier. </p></blockquote><p>This system use front-end and back-end separated web service architecture, based on Ubuntu Server OS.</p><p> Use <code>kismet</code> to monitor devices information, <code>aircrack-ng</code> handle de-auth attack, <code>NetworkManager</code> and <code>python-socket</code> controls connection. </p><h3 id="take-a-try-first">Take a try first</h3><pre class="hljs"><code><div>sudo airmon-ng start &lt;wlan_card&gt; # Enable monitor.
...
</div></code></pre><h3 id="attacker">Attacker</h3><p><strong>pyrcrack</strong> have a simple command to achieve de-auth attack:</p><pre class="hljs"><code><div><span class="hljs-comment"># Python</span>
...
</div></code></pre><p> So long as these bytes are read out and then rounded up every 10 bits into an integer, the joystick can be read again. </p><p>A example code for displaying the T1d signal value is as follows:</p><pre class="hljs"><code><div><span class="hljs-comment"># Python</span>
...
</div></code></pre><p>Mapping keys to the hex value:</p><pre class="hljs"><code><div><span class="hljs-comment"># Python</span>
...
</div></code></pre><p><strong>TelloPy</strong> gave a game controller stick mapper (function <code>__send_stick_command</code>) to control drone, so I motif it to adapt T1d: </p><pre class="hljs"><code><div><span class="hljs-comment"># Python</span>
...
</div></code></pre><p>Finally, we can connect to a drone and control ti with T1d:</p><pre class="hljs"><code><div><span class="hljs-comment"># Python</span>
<span class="hljs-keyword">import</span> struct
...
</div></code></pre><p> There are still some &quot;TODO:&quot; items to complete, such as the rub connection and changing the SSID; I won&#39;t go into detail in this blog, but here are some points: </p><ul><li><p> Ubuntu controls Wi-Fi via NetworkManager, I use d-bus to control NetworkManager. Create a multithreaded process that sends connection requests erratically during a de-auth attack. </p></li><li><p> The Tello SDK includes a Python example that demonstrates how to send a text command: <code>sock.sendto(&#39;wifi new ssid&gt; pwd&gt;&#39;, tello address)</code>. </p></li><li><p> The hole program has multiple job running in asynchronous, jobs need response a statue to frontend, use a task queue such as <code>flask-celery</code> will make schedule easier. </p></li></ul><h2 id="references--spacial-thanks">References &amp; Spacial thanks</h2><p><a href="https://github.com/NetworkManager/NetworkManager/blob/main/examples/python/dbus/add-wifi-psk-connection.py">https://github.com/NetworkManager/NetworkManager/blob/main/examples/python/dbus/add-wifi-psk-connection.py</a></p><p><a href="https://github.com/Diallomm/hack_GamesirT1d">https://github.com/Diallomm/hack_GamesirT1d</a></p><p><a href="https://github.com/hanyazou/TelloPy">https://github.com/hanyazou/TelloPy</a></p><p><a href="https://github.com/Matthias84/awesome-tello">https://github.com/Matthias84/awesome-tello</a></p></div></div>`,2),u=a({__name:"tello",setup(o){return(i,h)=>(n(),e(l,null,[r,t(s)],64))}});export{u as default};
